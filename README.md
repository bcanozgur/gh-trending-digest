...
**Serverless** — no always-on host. A daily GitHub Actions run pulls GitHub
Trending, ranks and curates the repos, emails the digest, and commits its state
back into the repo. Then the runner shuts down.

```mermaid
flowchart LR
    Trigger([Daily 10:30 or manual]) --> CLI
    Sources[(GitHub Trending + API)] --> CLI

    subgraph Runner [GitHub Actions · serverless]
        CLI[Notifier CLI<br/>fetch · score · curate · render]
    end

    CLI --> Email{{Resend email}}
    Email --> Inbox([Developer inbox])
    CLI <--> State[(data/ · state in git)]
```

`schedule` cron (10:30 Europe/Istanbul) and manual `workflow_dispatch` are the
only triggers. State lives as committed JSON under `data/` — no database. Email
delivery is adapter-based (`EMAIL_PROVIDER`: Resend / Brevo / SMTP).
